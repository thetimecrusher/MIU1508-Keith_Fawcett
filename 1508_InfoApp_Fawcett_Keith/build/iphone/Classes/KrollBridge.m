...
NSString * k1508_InfoApp_Fawcett_Keith$ModuleRequireFormat = @"(function(exports){"
...
@implementation k1508_InfoApp_Fawcett_KeithObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_k1508_infoapp_fawcett_keith);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	[context gc];
	[_k1508_infoapp_fawcett_keith gc];
}
...
{
	// create k1508_InfoApp_Fawcett_Keith global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "k1508_InfoApp_Fawcett_Keith" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_k1508_infoapp_fawcett_keith = [[k1508_InfoApp_Fawcett_KeithObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_k1508_infoapp_fawcett_keith];
	
	NSString *_k1508_infoapp_fawcett_keithNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _k1508_infoapp_fawcett_keithNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_k1508_infoapp_fawcett_keith valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:k1508_InfoApp_Fawcett_Keith$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.k1508_infoapp_fawcett_keith.kroll" 
