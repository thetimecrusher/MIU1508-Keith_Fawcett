...
- (NSString*)userAgent
{
	if (userAgent==nil)
	{
		...
		NSString *kk1508_InfoApp_Fawcett_KeithUserAgentPrefix = [NSString stringWithFormat:@"%s%s%s %s%s","Appc","eler","ator","Tita","nium"];
		userAgent = [[NSString stringWithFormat:@"%@/%s (%@)",kk1508_InfoApp_Fawcett_KeithUserAgentPrefix,TI_VERSION_STR,currentDeviceInfo] retain];
	}
	return userAgent;
}
...
+(NSDictionary *)tiAppProperties
{
    ...
    if(props == nil) {
        ...
        if(jsonData) {
            ...
            // If we have no data...
            // This should never happen on a k1508_InfoApp_Fawcett_Keith app using the node.js CLI
