...
//Rare exceptions to indicate a bug in the _k1508_utilityapp_fawcett_keith code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_k1508_utilityapp_fawcett_keith = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_k1508_utilityapp_fawcett_keith]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.k1508_utilityapp_fawcett_keith.module"
