...
NSString * fawcett_keith_project1$ModuleRequireFormat = @"(function(exports){"
...
@implementation fawcett_keith_project1Object
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_fawcett_keith_project1);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	// create fawcett_keith_project1 global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "fawcett_keith_project1" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_fawcett_keith_project1 = [[fawcett_keith_project1Object alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_fawcett_keith_project1];
	
	NSString *_fawcett_keith_project1NS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _fawcett_keith_project1NS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_fawcett_keith_project1 valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:fawcett_keith_project1$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.fawcett_keith_project1.kroll" 
